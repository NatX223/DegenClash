...
export async function computeGasLimitForResolver(
  provider: JsonRpcApiProvider,
  contract: Contract,
  proofData: proofGenerator.ContinuityResponse,
  signerAddress: string
 ...
 */
export async function submitProofToResolver(
  contract: Contract,
  proofData: proofGenerator.ContinuityResponse,
  gasLimit: bigint
): Promise<any> {
  ...
  return await contract.resolveMarket(
    chainKey,
    height,
    encodedTransaction,
    merkleRoot,
 ...
 */
export async function submitProofToResolverAndAwait(
  contract: Contract,
  proofData: proofGenerator.ContinuityResponse,
  gasLimit: bigint
): Promise<MintResult> {
  const response = await submitProofToResolver(contract, proofData, gasLimit);
...
// Export the new function for getting active game contracts
export { getActiveGameContracts } from './getActiveGameContracts';